...
def test_get_plugin_config_expects_plugin_config_path(mocker):
    ...
    mocker.patch(
        "unreal.PluginBlueprintLibrary.get_plugin_base_dir", return_value="/foo/bar"
    )
...
def test_get_plugin_config_plugin_config_not_found_expects_raises_PyCharmDebugRuntimeError(
    mocker,
):
    # Arrange
    from pycharmdebug.utils import get_plugin_config
...
def test_get_debug_port_plugin_config_not_found_expects_fallback_to_default_port(
    mocker,
):
    # Arrange
    from pycharmdebug.utils import get_debug_port, DEFAULT_PORT_NUMBER
...
def test_set_debug_port_expects_42_dumped_true_returned(mocker):
    ...
    # Assert
    mock_dump.assert_called_once_with({"port_number": 42}, mocker.ANY, indent=4)
    assert result is True


def test_set_debug_port_existing_data_in_config_expects_existing_data_maintained(
    mocker,
):
    # Arrange
    from pycharmdebug.utils import (
    ...
    mocker.patch("builtins.open")
    mocker.patch(
        "json.load", return_value={"port_number": DEFAULT_PORT_NUMBER, "foo": "bar"}
    )
    mock_dump = mocker.patch("json.dump")
    mocker.patch("pycharmdebug.utils.get_plugin_config")
...
def test_set_debug_port_incorrect_type_expects_raises_PyCharmDebugTypeError(mocker):
    ...
    mocker.patch("builtins.open")
    mocker.patch(
        "json.load", return_value={"port_number": 5678, "debug_egg": "/foo/bar.egg"}
    )
    mocker.patch("pycharmdebug.utils.get_plugin_config")
...
def test_set_debug_port_no_plugin_config_exists_expects_true_42_dumped_and_file_created(
    mocker,
):
    # Arrange
    from pycharmdebug.utils import set_debug_port
...
@pytest.mark.skipif(
    not sys.platform.startswith("win"), reason="This test is specific to Windows."
)
...
def test_find_system_dbg_egg_windows_cant_resolve_pycharm_installation_raises_PyCharmDebugRuntimeError(
    mocker,
):
    # Arrange
    ...
    mocker.patch(
        Path("pycharmdebug.utils.resolve_os_specific_pycharm_path"), return_value="win"
    )
...
def test_find_system_dbg_egg_cant_resolve_debug_egg_file_expects_raises_PyCharmDebugRuntimeError(
    mocker,
):
    # Arrange
    from pycharmdebug.utils import find_system_dbg_egg
...
def test_get_debug_egg_exists_in_config_expects_config_value_mocked_find_system_dbg_egg_not_called(
    mocker,
):
    # Arrange
...
def test_get_debug_egg_serialized_data_is_empty_string_expects_empty_string_returned(
    mocker,
):
    # Arrange
    from pycharmdebug.utils import get_debug_egg
...
def test_get_debug_egg_serialized_data_is_invalid_expects_PyCharmDebugRuntimeError_raised(
    mocker,
):
    # Arrange
    from pycharmdebug.utils import get_debug_egg
...
def test_set_debug_egg_config_doesnt_exist_gets_created_and_value_dumped_returns_true(
    mocker,
):
    # Arrange
    from pycharmdebug.utils import set_debug_egg
...
def test_set_debug_egg_config_has_other_data_expects_other_data_maintained(mocker):
    ...
    assert result is True
    mocked_dump.assert_called_once_with(
        {"foo": "bar", "debug_egg": "/foo/bar/pydevd-pycharm.egg"}, mocker.ANY, indent=4
    )
